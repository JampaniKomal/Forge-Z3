...
class Z3Engine:
    def __init__(self, topology: Topology):
        ...
        self.NetworkEdge = z3.Function("NetworkEdge", self.NodeSort, self.NodeSort, self.PortSort, z3.BoolSort())
        self.RunsCVE = z3.Function("RunsCVE", self.NodeSort, self.CveSort, z3.BoolSort())
        self.Reaches = z3.Function("Reaches", self.NodeSort, self.NodeSort, self.PortSort, z3.BoolSort())
    ...
    def _build_rules(self):
        ...
        # Rule 1: Reachability
        # An attacker can reach Node 2 if they have USER or ROOT on Node 1, and there is a physical edge.
        val_root = z3.BitVecVal(PRIVILEGE_MAP[PrivilegeLevel.ROOT], 3)
        val_user = z3.BitVecVal(PRIVILEGE_MAP[PrivilegeLevel.USER], 3)
        
        self.fp.rule(self.Reaches(n1, n2, port), [self.State(n1, val_root), self.NetworkEdge(n1, n2, port)])
        self.fp.rule(self.Reaches(n1, n2, port), [self.State(n1, val_user), self.NetworkEdge(n1, n2, port)])
    ...
    def _assert_facts(self):
        ...
        # The attacker is always Node 0 and starts with ROOT on their own machine
        self.fp.fact(self.State(z3.BitVecVal(0, 4), z3.BitVecVal(PRIVILEGE_MAP[PrivilegeLevel.ROOT], 3)))
